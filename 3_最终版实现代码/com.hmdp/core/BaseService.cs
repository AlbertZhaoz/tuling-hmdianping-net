...
namespace com.hmdp.repo
{
    public class BaseService<TEntity> : IBaseService<TEntity> where TEntity : class, new()
    {
        ...
        private ISqlSugarClient _db = DbScoped.SugarScope;
        ...
        public async Task<List<TEntity>> QueryByIDsWithExpression(Expression<Func<TEntity, bool>> expression)
        {
            //return await Task.Run(() => _db.Queryable<TEntity>().In(lstIds).ToList());
            return await _db.Queryable<TEntity>()
                .Where(expression)
        ...
        /// <param name="entity">博文实体类</param>
        /// <returns></returns>
        public async Task<long> AddNoSnow(TEntity entity)
        {
            //var i = await Task.Run(() => _db.Insertable(entity).ExecuteReturnBigIdentity());
            ////返回的i是long类型,这里你可以根据你的业务需要进行处理
            //return (int)i;
        ...
        /// <returns></returns>
        public async Task<long> AddAndIgnoreColumns(TEntity entity,Expression<Func<TEntity, object>> ignoreColumns )
        {
            var insert = _db.Insertable(entity)
        ...
        public async Task<int> Update(Expression<Func<TEntity, bool>> setColumns,
            Expression<Func<TEntity, bool>> whereColumns,
            List<string> lstIgnoreColumns = null)
        {
            bool ignoreColumns;
            return await _db.Updateable<TEntity>()
                ...
                .Where(whereColumns)
                .IgnoreColumns(lstIgnoreColumns.ToArray())
                .ExecuteCommandAsync();
        }
        ...
        /// <param name="entity">博文实体类</param>
        /// <returns></returns>
        public async Task<bool> DeleteNoEntity(Expression<Func<TEntity,bool>> whereExpression)
        {
        ...
        /// <returns></returns>
        public async Task<int> Count(Expression<Func<TEntity, bool>> whereString)
        {
            return await _db.Queryable<TEntity>().Where(whereString).CountAsync();
        }
